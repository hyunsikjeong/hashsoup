...
class RIPEMD128(Hasher):
    ...
    CONST_C1 = [
        0, 0x5a827999, 0x6ed9eba1, 0x8f1bbcdc
    ]
    ...
    def _update_chunk(self, chunk: bytes):
        ...
        for i in range(64):
            ...
            AA = rol32(AA, self.s2[i])
            AA, DD, CC, BB = DD, CC, BB, AA

        self.state = [
            (self.state[1] + C + DD) & 0xFFFFFFFF,
            (self.state[2] + D + AA) & 0xFFFFFFFF,
            (self.state[3] + A + BB) & 0xFFFFFFFF,
